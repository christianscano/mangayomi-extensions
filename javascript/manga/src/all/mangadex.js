const mangayomiSources = [
  {
    ...
    iconUrl:
      "https://raw.githubusercontent.com/christianscano/mangayomi-extensions/main/javascript/icon/all.mangadex.png",
    typeSource: "single",
    itemType: 0,
    version: "0.1.4",
...
class DefaultExtension extends MProvider {
  ...
  async getDetail(url) {
    ...
    manga.description =
      data.attributes.description[this.source.lang] ??
    ...
    manga.status = { ongoing: 0, completed: 1, hiatus: 2, cancelled: 3 }[
      data.attributes.status
    ];
    const mangaId = url.split("/").pop();
    const chapterData = await this.fetchPaginatedChapters(
      mangaId,
      this.source.lang,
    );
