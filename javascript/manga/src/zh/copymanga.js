const mangayomiSources = [
  {
    ...
    iconUrl:
      "https://raw.githubusercontent.com/christianscano/mangayomi-extensions/main/javascript/icon/zh.copymanga.png",
    typeSource: "single",
    itemType: 0,
    isNsfw: false,
    version: "0.0.25",
...
class DefaultExtension extends MProvider {
  ...
  reqHeaders() {
    ...
    return {
      "User-Agent":
        "duoTuoCartoon/3.2.4 (iPhone; iOS 18.0.1; Scale/3.00) iDOKit/1.0.0 RSSX/1.0.0",
  ...
  async getPopular(page) {
    return await this.getManga(
      `/api/v3/comics?free_type=1&limit=16&offset=${(page - 1) * 16}&ordering=-popular&_update=true`,
    );
  }

  async getLatestUpdates(page) {
    return await this.getManga(
      `/api/v3/comics?free_type=1&limit=16&offset=${(page - 1) * 16}&ordering=-datetime_updated&_update=true`,
    );
  }
  ...
  async search(query, page, filters) {
    if (query != "") {
      const res = await new Client().get(
        this.source.apiUrl +
          `/api/v3/search/comic?platform=1&q=${query}&limit=16&offset=${(page - 1) * 16}&q_type=&_update=true`,
        this.reqHeaders(),
      );
    ...
    return await this.getManga(
      `/api/v3/comics?free_type=1&limit=16&offset=${(page - 1) * 16}&theme=${type}&top=${region}&ordering=${sort}&_update=true`,
    );
  }
  ...
  async getDetail(url) {
    ...
    const res = await new Client().get(
      this.source.apiUrl + `/api/v3/comic2/${url}`,
      this.reqHeaders(),
    );
    ...
    const chapters = [];
    const res_ = await new Client().get(
      this.source.baseUrl + `/comicdetail/${url}/chapters`,
      {
        "User-Agent":
  ...
  async getPageList(url) {
    ...
    const res = await new Client().get(
      this.source.baseUrl + `/comic/${urls[0]}/chapter/${urls[1]}`,
      {
        "User-Agent":
          "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/117.0.0.0 Safari/537.36",
      },
