const mangayomiSources = [
  {
    ...
    apiUrl: "",
    iconUrl:
      "https://raw.githubusercontent.com/christianscano/mangayomi-extensions/main/javascript/icon/zh.gfmanhua.png",
    typeSource: "single",
    itemType: 0,
    version: "0.0.1",
...
class DefaultExtension extends MProvider {
  ...
  async getDetail(url) {
    ...
    let status = 0;
    if (
      this.stringUTF8(infos[0].selectFirst("a").text.search("连载中")) == -1
    ) {
      status = 1;
    }
  ...
  async getPageList(url) {
    ...
    const chapterImages = chapterImagesMatch
      ? JSON.parse(chapterImagesMatch[1])
      : [];
    const pageImageMatch = script.match(/var chapterPath = "(.*?)";/);
    const pageImage = pageImageMatch ? pageImageMatch[1] : "";
    for (let i = 0; i < chapterImages.length; i++) {
      chapterImages[i] =
        "https://res.xiaoqinre.com/" + pageImage + chapterImages[i];
    }
