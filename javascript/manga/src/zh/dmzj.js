const mangayomiSources = [
  {
    ...
    apiUrl: "",
    iconUrl:
      "https://raw.githubusercontent.com/christianscano/mangayomi-extensions/main/javascript/icon/zh.dmzj.png",
    typeSource: "single",
    itemType: 0,
    isNsfw: false,
    version: "0.0.3",
...
class DefaultExtension extends MProvider {
  ...
  async getPopular(page) {
    return await this.getManga(
      `https://m.idmzj.com/classify/0-0-0-0-0-${page - 1}.json`,
    );
  }
  async getLatestUpdates(page) {
    return await this.getManga(
      `https://m.idmzj.com/classify/0-0-0-0-1-${page - 1}.json`,
    );
  }
  async search(query, page, filters) {
    if (query == "") {
      ...
      return await this.getManga(
        `https://m.idmzj.com/classify/${type}-0-${status}-${region}-${sort}-${page - 1}.json`,
      );
    }
    const res = await new Client().get(
      `http://sacg.dmzj.com/comicsum/search.php?s=${query}`,
    );
  ...
  async getDetail(url) {
    ...
    const res = await new Client().get(
      `https://www.dmzj.com/api/v1/comic1/comic/detail?comic_py=${url}&channel=pc&app_name=dmzj&version=1.0.0&uid=${preference.get("uid")}`,
    );
    const datas = JSON.parse(res.body);
    if (datas["errno"] != 0) {
      return {
      ...
      for (const chlist of datas["data"]["comicInfo"]["chapterList"]) {
        for (const ch of chlist["data"]) {
          chapters.push({
            ...
            url:
              datas["data"]["comicInfo"]["id"].toString() +
              "|" +
              ch["chapter_id"].toString(),
  ...
  async getPageList(url) {
    ...
    const ids = url.split("|");
    const res = await new Client().get(
      `https://www.dmzj.com/api/v1/comic1/chapter/detail?channel=pc&app_name=dmzj&version=1.0.0&comic_id=${ids[0]}&chapter_id=${ids[1]}&uid=${preference.get("uid")}`,
    );
    const datas = JSON.parse(res.body);
    if (datas["errno"] != 0) {
      return [];
