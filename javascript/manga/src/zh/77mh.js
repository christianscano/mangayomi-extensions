const mangayomiSources = [
  {
    ...
    apiUrl: "",
    iconUrl:
      "https://raw.githubusercontent.com/christianscano/mangayomi-extensions/main/javascript/icon/zh.77mh.png",
    typeSource: "single",
    itemType: 0,
    isNsfw: false,
    version: "0.0.35",
...
class DefaultExtension extends MProvider {
  ...
  async getIndex2(url) {
    ...
    for (const element of elements) {
      const title = element
        ...
        .replace("</em>", "");
      const url =
        "/" + element.selectFirst("h1 a").attr("href").split("/").slice(-1)[0];
      const cover = element.selectFirst("img").attr("src");
      mangas.push({
        name: title,
        link: url,
  ...
  async getLatestUpdates(page) {
    return await this.getIndex1(
      `${this.getBaseUrl()}/lianzai/index_${page - 1}.html`,
    );
  }
