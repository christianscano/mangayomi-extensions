const mangayomiSources = [
  {
    ...
    apiUrl: "",
    iconUrl:
      "https://raw.githubusercontent.com/christianscano/mangayomi-extensions/main/javascript/icon/zh.yhdm.png",
    typeSource: "single",
    itemType: 1,
    isNsfw: false,
    version: "0.0.2",
...
class DefaultExtension extends MProvider {
  ...
  async getLatestUpdates(page) {
    return await this.getItems(
      `/${new Date().getFullYear()}/${page == 1 ? "" : page.toString() + ".html"}`,
      "div.lpic li",
      true,
    );
  }
  ...
  async getDetail(url) {
    ...
    const genre = doc
      .select("div.sinfo a")
      .map((e) => this.stringUTF8(e.text, true));
    genre.splice(-1);
  ...
  async getVideoList(url) {
    const res = await new Client().get(this.getBaseUrl() + url);
    const doc = new Document(res.body);
    const video_url = this.stringUTF8(
      doc.selectFirst("div#playbox").attr("data-vid").split("$")[0],
      true,
    );
    return [
      {
