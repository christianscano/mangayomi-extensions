const mangayomiSources = [
  {
    ...
    apiUrl: "",
    iconUrl:
      "https://raw.githubusercontent.com/christianscano/mangayomi-extensions/main/javascript/icon/es.animeflv.png",
    typeSource: "single",
    itemType: 1,
    isNsfw: false,
    version: "0.0.1",
...
class DefaultExtension extends MProvider {
  async getPopular(page) {
    ...
    const res = await new Client().get(
      `${baseUrl}/browse?order=rating&page=${page}`,
    );
  ...
  async getLatestUpdates(page) {
    const baseUrl = this.source.baseUrl;
    const res = await new Client().get(
      `${baseUrl}/browse?order=added&page=${page}`,
    );
  ...
  async getDetail(url) {
    ...
    for (const script of document.select("script")) {
      if (script.text.includes("var anime_info =")) {
        const animeInfo = script.text
          .substringAfter("var anime_info = [")
          .substringBefore("];");
        const arrInfo = JSON.parse(`[${animeInfo}]`);
        const animeUri = arrInfo[2].replace(/"/g, "");
        const episodes = script.text
  ...
  async getVideoList(url) {
    ...
    const responseString = jsonString
      .substringAfter("var videos =")
      .substringBefore(";")
      .trim();
    ...
    for (const item of serverModel.SUB) {
      ...
      switch (item.title) {
        ...
        case "YourUpload":
          ...
          break;
        case "SW":
          videoList = await streamWishExtractor(
            item.url || item.code,
            "StreamWish:",
          );
          break;
  ...
  sortVideos(videos) {
    ...
    videos.sort((a, b) => {
      let qualityMatchA = 0;
      if (a.quality.includes(server) && a.quality.includes(quality)) {
        qualityMatchA = 1;
      }
      let qualityMatchB = 0;
      if (b.quality.includes(server) && b.quality.includes(quality)) {
        qualityMatchB = 1;
      }
  ...
  animeFromElement(body) {
    const elements = new Document(body).select(
    ...
    for (const element of elements) {
      ...
      const thumbnailUrl = element
        .selectFirst("a div.Image figure img")
        .attr("src");
      const imageUrl = thumbnailUrl
        ? thumbnailUrl
        : element.selectFirst("a div.Image figure img").attr("data-cfsrc");
    ...
    return {
      list: list,
      hasNextPage:
        new Document(body).select("div.Container ul.ListAnimes li article")
          .length > 0,
    };
  }
