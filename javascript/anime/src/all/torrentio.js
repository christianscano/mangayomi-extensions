const mangayomiSources = [
  {
    ...
    apiUrl: "",
    iconUrl:
      "https://raw.githubusercontent.com/christianscano/mangayomi-extensions/main/javascript/icon/all.torrentio.png",
    typeSource: "torrent",
    isManga: false,
    itemType: 1,
    version: "0.0.25",
...
class DefaultExtension extends MProvider {
  ...
  parseSearchJson(jsonLine) {
    ...
    const hasNextPage =
      popularTitlesResponse?.data?.popularTitles?.pageInfo?.hasNextPage ||
      false;
    ...
    const animeList = edges
      .map((edge) => {
        ...
        return {
          ...
          genre:
            content.genres?.map((genre) => genre.translation).filter(Boolean) ||
            [],
          author: (
            content.credits
  ...
  async search(query, page, filters) {
    return this.parseSearchJson(
      (await this.searchAnimeRequest(page, query)).body,
    );
  }
  async getDetail(url) {
    const anime = {};
      ...
      switch (meta.type) {
        case "show":
          ...
          return videos
            .filter(
              (video) =>
                (video.firstAired ? new Date(video.firstAired) : Date.now()) <
                Date.now(),
            )
            .map((video) => {
              const firstAired = video.firstAired
                ? new Date(video.firstAired)
                : Date.now();
            ...
            .sort((a, b) => {
              const seasonA = parseInt(
                a.name.substringAfter("S").substringBefore(":"),
                10,
              );
              const seasonB = parseInt(
                b.name.substringAfter("S").substringBefore(":"),
                10,
              );
              const episodeA = parseInt(
                a.name.substringAfter("E").substringBefore(" -"),
                10,
              );
              const episodeB = parseInt(
                b.name.substringAfter("E").substringBefore(" -"),
                10,
              );
  ...
  appendQueryParam(key, values) {
    ...
    if (values && values.length > 0) {
      const filteredValues = Array.from(values)
  ...
  async getVideoList(url) {
    ...
    mainURL += this.appendQueryParam(
      "providers",
      preferences.get("provider_selection1"),
    );
    mainURL += this.appendQueryParam(
      "language",
      preferences.get("lang_selection"),
    );
    mainURL += this.appendQueryParam(
      "qualityfilter",
      preferences.get("quality_selection"),
    );
    mainURL += this.appendQueryParam(
      "sort",
      new Set([preferences.get("sorting_link")]),
    );
    mainURL += url;
    ...
    return [
      {
        ...
        listPreference: {
          ...
          summary:
            "⚠️ Increasing the number of links will increase the loading time of the video list",
          valueIndex: 1,
      ...
      },
      {
        ...
        switchPreferenceCompat: {
          ...
          summary:
            "Codec: (HEVC / x265)  & AV1. High-quality video with less data usage.",
          value: false,
        },
      },
      {
