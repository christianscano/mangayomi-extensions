...
class DefaultExtension extends MProvider {
  ...
  async getDetail(url) {
    ...
    const responseEpisodes = await this.client.get(
      `https://anime-kitsu.strem.fun/meta/series/kitsu%3A${kitsuId}.json`,
    );
          ...
          return videos
            .filter(
              (video) =>
                ...
                (video.released ? new Date(video.released) : Date.now()) <
                  Date.now(),
            )
            .map((video) => {
              const releaseDate = video.released
                ? new Date(video.released)
                : Date.now();
              return {
  ...
  async getVideoList(url) {
    ...
    mainURL += this.appendQueryParam(
      "providers",
      preferences.get("provider_selection"),
    );
    mainURL += this.appendQueryParam(
      "language",
